...
class ViewActivities extends React.Component {
    ...
    componentDidMount() {
		...
		this.setState({viewActivities: viewActivities});
	}
    ...
    render() {
        return(
            <Layout>
                <div className="col-md-11" id="content">
                    <Navbar expand="lg" className="flex-grow-1 py-0">
                        <Nav className="menuItems flex-row flex-grow-1">
                            ...
                            <div className="activeFontColor col-md-11 smallText"><p className="text-left">{this.state.companyName ? this.state.companyName:'Dezign Code'}</p>
                                <p className="normalText fontColor text-left">View Activities</p>
                            </div>
                            <Nav.Link className="mr-2">
                            ...
                            <Nav.Link className="mr-2">
                                <OverlayTrigger trigger="click" placement="bottom-end" overlay={settingPopover} id="popover-setting">
                                    <Icomoon icon="gear"  size={22} className="align-self-center"/>
                                </OverlayTrigger>
                            </Nav.Link>
                            <Nav.Link className="mr-2">
                                <Profile/>
                ...
                {this.renderViewActivities()}
            </Layout>
        )
    }

    // Render View Activities page
                        ...
                        <div className="col-md-12 borderRadius bg-white py-4" style={{height:550}}>
                            ...
                            <Row className="my-0 py-0 px-5">
                                <div className="border-left borderWidth rounded scrollStyle" style={{height:420}}>
                                    {this.state.viewActivities.map((viewActivities) => (
                                        <div className="mx-0 row"  key={viewActivities.id}>
                                            <div className="col-md-1  border-left leftBorderStyle"> 
                                                <hr className="bg-dark pl-3 mt-4"></hr> 
                                            </div>
                                            <div className="col-md-11">
                                                <p className="smallText my-0 py-0 text-dark text-justify">{viewActivities.content} </p>
                                                <div className='col-md-9 d-flex justify-content-between ml-0 pl-0 mt-2'>
                                                    <p className="xSmallText">{moment(viewActivities.date).format('DD-MM-YYYY HH:mm:ss a')}</p>
                                                    <p className="xSmallText">{viewActivities.time}</p>
                                                    <p className="xSmallText">{viewActivities.mail}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
